$(document).ready(function () {
  const theremin = $('<div class="theremin"></div>');
  document.body.appendChild(theremin[0]);
  ...
  theremin.on("mousemove", function (e) {
    if (!audioCtx) {
    ...
    const { top, right, left, bottom } = theremin[0].getBoundingClientRect();
    const pitch = ((e.clientX - left) / right) * MAX_FREQUENCY;
    const volume = (1 - (e.clientY - top) / bottom) * MAX_VOL;

    oscillatorNode.frequency.value = pitch;
    gainNode.gain.value = volume;
  });

  theremin.on("mouseout", function (e) {
    oscillatorNode.stop();
    oscillatorNode = null;
  });
});
